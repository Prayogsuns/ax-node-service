module "custom-iq-dispatch-adapter-service" {
  ...
  // HPA Config
  // hpa-enabled        = contains(local.services-enabled, "iq-dispatch-adapter-service") ? "true" : "false"
  hpa-enabled        = "true"
  metric             = "cpu"
  ...
  ingress-rules = [{
    http = {
      paths = [{
        backend = {
          serviceName = "custom-iq-dispatch-adapter-service"
          servicePort = local.iq-dispatch-adapter-service-config["port"]
        }
        path = "/iq-dispatch-adapter"
      }]
    }
  }]
